#!/usr/bin/env python3
"""
Search-engine click bot for irisvision.ai.

Runs `site:irisvision.ai` queries on Google / Yahoo / Bing and counts how
many result links it can open. Saves daily totals to:
...
_HEADER = ["date", "run_timestamp", "site", "engine", "clicks"]


def save_daily_clicks(site: Site, results: dict, base_output: str = "seo_reports") -> Path:
    """Append per-engine totals to the per-site daily CSV."""
    now = datetime.utcnow()
...
def run_all(engines: list[str] = DEFAULT_ENGINES) -> dict[str, dict]:
    site = SITE
...
def _main() -> None:
    ap = argparse.ArgumentParser(description="irisvision.ai search-engine click bot")
    ap.add_argument(
        "--engines",
