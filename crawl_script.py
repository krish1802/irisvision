#!/usr/bin/env python3
"""
Daily SEO crawler for irisvision.ai.
...
No API keys, no logins, no third-party services. Pure public-web fetching.
...
from sites_config import SITE, Site


HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; SEO-Audit-Bot/1.0)"}

# Static-asset extensions to skip when discovering links — these aren't pages.
_SKIP_EXTS = (
    ".woff", ".woff2", ".ttf", ".otf", ".eot",
    ".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp", ".ico", ".avif",
    ".css", ".js", ".mjs", ".map",
    ".pdf", ".zip", ".gz", ".mp4", ".mp3", ".webm", ".mov",
    ".xml", ".json", ".txt", ".webmanifest",
)


def _is_crawlable(url: str) -> bool:
    """Return False for static assets and Next.js build artifacts."""
    low = url.lower().split("?", 1)[0].split("#", 1)[0]
    if low.endswith(_SKIP_EXTS):
        return False
    if "/_next/static/" in low or "/_next/data/" in low:
        return False
    if "/cdn-cgi/" in low:
        return False
    return True
...
def run_site(
    ...
    # Keyword analysis (200 OK pages only)
    ...
    if kw_rows:
        ...
        # Clusters seeded from tracked keywords + observed h2 phrases
...
def _main() -> None:
    ap = argparse.ArgumentParser(description="irisvision.ai daily SEO crawler")
    ...
    print("\n──────── CRAWL SUMMARY ────────")
    print(
        f"  {s['domain']:25s}  pages={s['audit_pages']:4d} "
        f"issues={s['audit_issues']:4d} kw_pages={s['keyword_pages']:3d}"
    )
    print(f"\n🎉 Done. Results in {args.output}/{SITE.slug}/")
